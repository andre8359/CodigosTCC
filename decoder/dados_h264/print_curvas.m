...
for x=1:1:length(w)
    ...
    bitrate_SP(2,:) = fliplr(temp(1:36,3)');
    bitrate_SP(1,:) = fliplr(temp(37:end,3)');

    temp = load(['bitrates_' videoID '_original.txt']);
    bitrate_ori  = fliplr(temp(1:36,2)');
    ...
    title(['H.264 ' videoID ' Fator M = 2  '],'fontweight','bold','fontsize',16);
    ...
    grid on;
    box on;
    print(['H264_' videoID '_Downsampling_factor_2'],'-dpng')
    ...
    axis([0 0.3 22 45])
     h=get (gcf, 'currentaxes');
    set(h,'fontweight','bold','linewidth',2)
    title(['h.264 ' videoID ' Fator M = 4  '],'fontweight','bold','fontsize',16);
    ...
    print(['H264_' videoID '_Downsampling_factor_4'],'-dpng')
end